...
def test_get_compsumption_timeseries(mocker, data_regression, novafos):
    ...
    mock_response = requests.Response()
    mock_response.status_code = 200
    mock_response._content = utils.load_data("consumption_day_data_water_zoom_2.json")
    mock_post.return_value = mock_response

    actuals = novafos._get_consumption_timeseries(novafos._active_meters[0], "2024-12-01", "2024-12-31")
...
def test_get_all_compsumption_timeseries(mocker, data_regression, novafos):
    ...
    actuals = novafos._get_all_consumption_timeseries("2024-12-01", "2024-12-31")
    data_regression.check(actuals)
