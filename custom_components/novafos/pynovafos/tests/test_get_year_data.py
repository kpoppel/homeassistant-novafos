...
def test_get_year_data(mocker, data_regression, novafos):
    prepare_active_meters(mocker, novafos)

    mock_post = mocker.patch("requests.post")
    mock_response_1 = requests.Response()
    mock_response_1.status_code = 200
    mock_response_1._content = utils.load_data("consumption_year_data_water_zoom_0.json")

    mock_response_2 = requests.Response()
    mock_response_2.status_code = 200
    mock_response_2._content = utils.load_data("consumption_year_data_heating_zoom_0.json")

    mock_post.side_effect = [mock_response_1, mock_response_2]

    novafos._get_year_data()
    data_regression.check([novafos._last_valid_day, novafos._meter_data])

def test_get_year_data_valid_date_ok(mocker, data_regression, novafos):
    prepare_active_meters(mocker, novafos)
